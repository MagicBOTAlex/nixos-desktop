{ lib, rustPlatform, fetchFromGitHub, pkg-config, wrapGAppsHook3, atk, bzip2
, cairo, dbus, gdk-pixbuf, glib, gst_all_1, gtk3, libayatana-appindicator
, libgit2, libpulseaudio, libsodium, libXtst, libvpx, libyuv, libopus, libaom
, libxkbcommon, libsciter, xdotool, pam, pango, zlib, zstd, stdenv, alsa-lib
...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  nativeBuildInputs =
    [ copyDesktopItems pkg-config rustPlatform.bindgenHook wrapGAppsHook3 ];

  buildFeatures =
    lib.optionals stdenv.hostPlatform.isLinux [ "linux-pkg-config" ];
  ...
  buildInputs = [
    ...
    ++ lib.optionals stdenv.hostPlatform.isLinux [ alsa-lib xdotool ];
  ...
  meta = {
    ...
    changelog =
      "https://github.com/rustdesk/rustdesk/releases/tag/${finalAttrs.version}";
    license = lib.licenses.agpl3Only;
    maintainers = with lib.maintainers; [ ocfox leixb ];
